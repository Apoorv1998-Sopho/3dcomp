...
for pathI in paths:
    ...
    d = {} # for storing sift discriptors
    for i in range(len(epilines)):
        ...
        discriptors = findCustomDiscriptor(I2, vPts, d, listOfPoints, channel=channel, method=method)
        if discriptors == None:
        	continue
        if discriptors != None:
            ptI1 = listOfPoints[i]
            I1ptDiscriptor = findCustomDiscriptor(I1, [ptI1], d, listOfPoints, channel=channel, method=method) # can return empty array
